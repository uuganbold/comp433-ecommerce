...
class Api {
    ...
    protected async sendURIRequest<T>(uri: string, method = HttpMethods.GET, request?: IRequest): Promise<T> {
        const response = await this.server.sendURIRequest(uri, method, request);
    ...
    protected async sendURIRequestWithVoidResponse<T>(uri: string, method = HttpMethods.GET, request?: IRequest): Promise<void> {
        const response = await this.server.sendURIRequest(uri, method, request);
